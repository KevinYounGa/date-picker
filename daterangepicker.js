...
!function($, moment) {
	...
	var DateRangePicker = function(element, options, cb) {
		...
		var DRPTemplate = '<div class="daterangepicker dropdown-menu">'
				...
				+ '<div class="ranges" style="width:100%;text-align:center" >' +
	...
	DateRangePicker.prototype = {
		...
		show : function(e) {
			...
			$(document).on('click.daterangepicker',
					$.proxy(this.outsideClick, this));
			// also explicitly play nice with Bootstrap dropdowns, which stopPropagation when clicking them
			$(document).on('click.daterangepicker', '[data-toggle=dropdown]',
					$.proxy(this.outsideClick, this));
		...
		updateCalendars : function() {
			...
			this.rightCalendar.calendar = this
					.buildCalendar(this.rightCalendar.month.month() + 1,
			...
			this.container.find('.calendar.left').html(
					this.renderCalendar("left", this.leftCalendar.calendar,
							this.startDate, this.minDate, this.maxDate));
			this.container.find('.calendar.right').html(
					this.renderCalendar("right", this.rightCalendar.calendar,
				...
				if (this.timePicker) {
					if (this.startDate.isSame(this.ranges[range][0])
						...
						this.chosenLabel = this.container.find(
								'.ranges li:eq(' + i + ')').addClass('active')
								.html();
					}
				} else {
		...
		renderCalendar : function(leftORright, calendar, selected, minDate,
				maxDate) {
			...
			/* if (this.showWeekNumbers)
			     html += '<th></th>';
			 if(calendar[1][1].month()){
			 	alert(this.locale.monthNames[calendar[1][1].month()]);
			 }*/
			...
			if (leftORright == "left") {
				...
				var month=date.getMonth();
				 //alert(month);
					if (!minDate&&(calendar[1][1].month()>month)) {
				...
				for ( var row = 0; row < 6; row++) {
					...
					for ( var col = 0; col < 7; col++) {
						...
						var id= 'r' + row + 'c' + col;
						var cname = ' ';
						cname += (calendar[row][col].isBefore(selected
								.format('YYYY-MM-DD'))) ? 'offleft' : '';
						if (calendar[row][col].isBefore(selected.format('YYYY-MM-DD'))||(calendar[row][col].month() != calendar[1][1].month())) {
							//将不是本月的隐藏
							cname = ' offleft';
						}
						if (calendar[row][col].format('YYYY-MM-DD') == selected.format('YYYY-MM-DD')) {
							cname += ' active newToday';
						}
						html += '<td  id='+id+' class="'+ cname.replace(/\s+/g, ' ').replace(/^\s?(.*?)\s?$/, '$1')
						+ '" data-title="' + title + '"  >';
						html += calendar[row][col].date();
						if (calendar[row][col].isAfter(selected.format('YYYY-MM-DD'))&&(calendar[row][col].month() == calendar[1][1].month())) {
							//将不是本月的隐藏
							html += '<div style="color:red">' + "￥33";
							html += '</div>';
							html += '<div style="color:#C0D9D9">' + "剩22";
							html += '</div>';
						}
						html += '</td>';
					}
			...
			$("td.active.newToday").html("今天");
			if (leftORright == "right") {
				//右边
				var dateHtml = calendar[1][1].format("YYYY") + "年"+ "&nbsp&nbsp"+ this.locale.monthNames[calendar[1][1].month()];
				if (this.showDropdowns) {
					dateHtml = this.renderDropdowns(calendar[1][1], minDate,
							maxDate);
				}
				...
				for ( var row = 0; row < 6; row++) {
					...
					for ( var col = 0; col < 7; col++) {
						var cname = '';
						cname += (calendar[row][col].month() == calendar[1][1].month()) ? '' : 'offright';
						if (cname == " offright") {
							//将不是本月的隐藏
							$("td.offright").css("color", "white");
						}

						/* else if (calendar[row][col].format('YYYY-MM-DD') == selected.format('YYYY-MM-DD')) {
						 	if(leftORright=="left"){cname += ' active ';}
